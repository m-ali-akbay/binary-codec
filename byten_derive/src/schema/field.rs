...
impl BinarySchema for NamedFieldsSchema {
    ...
    fn measure_fixed(&self) -> syn::Result<proc_macro2::TokenStream> {
        let measures = self
            .fields
            .iter()
            .map(|(_, schema)| schema.measure_fixed())
            .collect::<syn::Result<Vec<_>>>()?;
        Ok(quote! {
            0 #( + #measures )*
        })
...
impl BinarySchema for UnnamedFieldsSchema {
    fn decode(&self, ctx: &DecodeContext) -> syn::Result<proc_macro2::TokenStream> {
        let fields = self
            .fields
            .iter()
            .map(|schema| schema.decode(&ctx.clone()))
            .collect::<syn::Result<Vec<_>>>()?;
        Ok(quote! { ( #(#fields),* ) })
    }
    ...
    fn measure_fixed(&self) -> syn::Result<proc_macro2::TokenStream> {
        let measures = self
            .fields
            .iter()
            .map(|schema| schema.measure_fixed())
            .collect::<syn::Result<Vec<_>>>()?;
        Ok(quote! {
            0 #( + #measures )*
        })
