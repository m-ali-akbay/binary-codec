...
use super::{BinarySchema, DecodeContext, EncodeContext, FieldsSchema, MeasureContext, build_codec_schema, interpret_fields_schema};

pub fn interpret_enum_schema(input: &DeriveInput) -> syn::Result<Box<dyn BinarySchema>> {
    let Data::Enum(ref data) = input.data else {
        return Err(syn::Error::new(input.span(), "EnumSchema can only be created from enum data"));
    };

    let repr = input.attrs.iter().find(
        |attr| attr.path().is_ident("repr")
    ).ok_or_else(|| {
        syn::Error::new(input.span(), "Enum must have a repr attribute")
    })?;
    let repr = match &repr.meta {
        Meta::List(meta) => {
            meta.parse_args::<Type>()?
        },
        _ => {
            return Err(syn::Error::new(repr.span(), "Invalid repr attribute format"));
        }
    };
    ...
    let variants = data.variants.iter().map(|variant| {
        ...
        let discriminant = match &variant.discriminant {
            ...
            None => {
                return Err(syn::Error::new(variant.span(), "Enum variants must have discriminants"));
            },
        };
...
impl BinarySchema for EnumSchema {
    ...
    fn measure_fixed(&self) -> syn::Result<proc_macro2::TokenStream> {
        Err(syn::Error::new(self.ident.span(), "Fixed measure is not yet supported for enums"))
    }
}
